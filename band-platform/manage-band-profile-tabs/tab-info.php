...
global $band_post_title, $band_post_content, $display_band_name, $display_band_bio;
...
<div class="band-profile-content-card">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1em; border-bottom: 1px solid #555; padding-bottom: 0.75em;">
        <h2 style="margin-bottom:0; padding-bottom:0;border:none;"><?php esc_html_e( 'Band Info', 'extra-chill-community' ); ?></h2>
        <?php 
        ...
        if ( $edit_mode && $target_band_id > 0 ) : 
            ...
            $label = ( ! empty( $link_page_id ) && get_post_status( $link_page_id ) ) ? __( 'Manage Link Page', 'extra-chill-community' ) : __( 'Create Link Page', 'extra-chill-community' );
        ?>
        ...
        <?php else : // Create mode or conditions not met for edit mode button ?>
            <a href="#" class="button button-secondary disabled" style="pointer-events: none; opacity: 0.6;" title="<?php esc_attr_e( 'Save your band profile first. A link page will be created automatically.', 'extra-chill-community' ); ?>" onclick="return false;">
                <?php esc_html_e( 'Create Link Page', 'extra-chill-community' ); ?>
            </a>
        <?php endif; ?>
    </div>
    ...
    <div class="form-group">
        <label><?php echo $edit_mode ? esc_html__( 'Profile Picture', 'extra-chill-community' ) : esc_html__( 'Profile Picture (Optional)', 'extra-chill-community' ); ?></label>
        ...
        <div id="featured-image-preview-container" class="current-image-preview featured-image-preview">
            ...
            ?>
            <img id="featured-image-preview-img" src="<?php echo esc_url($current_featured_image_url); ?>" alt="<?php esc_attr_e('Profile picture preview', 'extra-chill-community'); ?>" style="<?php echo esc_attr($featured_image_style); ?>">
            ...
            if ($show_no_image_notice): ?>
                 <p class="no-image-notice"><?php esc_html_e( 'No image available.', 'extra-chill-community' ); ?></p>
            <?php endif; ?>
        </div>
        ...
        <?php endif; ?>
        <p class="description"><?php echo $edit_mode ? esc_html__( 'This picture is also used for your Extrachill.link page. ', 'extra-chill-community' ) : esc_html__( 'Upload an image for your band profile (e.g., logo, band photo). Your user avatar will be used if no image is uploaded.', 'extra-chill-community' ); ?></p>
    </div>

    <!-- Band Header Image -->
    <div class="form-group">
        <label><?php esc_html_e( 'Band Forum Header Image', 'extra-chill-community' ); ?></label>
        ...
        <div id="band-header-image-preview-container" class="current-image-preview band-header-image-preview">
            ...
            ?>
            <img id="band-header-image-preview-img" src="<?php echo esc_url($preview_image_src); ?>" alt="<?php esc_attr_e('Header image preview', 'extra-chill-community'); ?>" style="<?php echo esc_attr($preview_image_style); ?>">
            
            <?php if ($edit_mode && !$current_header_image_id): ?>
                 <p class="no-image-notice"><?php esc_html_e( 'No header image set.', 'extra-chill-community' ); ?></p>
            <?php endif; ?>
        </div>

        <input type="file" id="band_header_image" name="band_header_image" accept="image/*">
        <p class="description"><?php esc_html_e( 'Recommended aspect ratio: 16:9. This image appears at the top of your band\'s public profile page.', 'extra-chill-community' ); ?></p>
    </div>

    <!-- Band Name -->
    <div class="form-group">
        <label for="band_title"><?php esc_html_e( 'Band Name *', 'extra-chill-community' ); ?></label>
        <input type="text" id="band_title" name="band_title" required value="<?php echo esc_attr( $display_band_name ); ?>">
    </div>

    <!-- City / Region -->
    <div class="form-group">
        <label for="local_city"><?php esc_html_e( 'City / Region', 'extra-chill-community' ); ?></label>
        <input type="text" id="local_city" name="local_city" value="<?php echo esc_attr( $current_local_city ); ?>" placeholder="e.g., Austin, TX">
    </div>

    <div class="form-group">
        <label for="genre"><?php esc_html_e( 'Genre', 'extra-chill-community' ); ?></label>
        <input type="text" id="genre" name="genre" value="<?php echo esc_attr( $current_genre ); ?>" placeholder="e.g., Indie Rock, Electronic, Folk">
    </div>
    
    <div class="form-group">
        <label for="band_bio"><?php esc_html_e( 'Band Bio', 'extra-chill-community' ); ?></label>
        <textarea id="band_bio" name="band_bio" rows="10"><?php echo esc_textarea( $display_band_bio ); ?></textarea>
        <p class="description extrch-sync-info"><small><?php esc_html_e( 'This bio is also used for your Extrachill.link page.', 'extra-chill-community' ); ?></small></p>
    </div>
</div>
...
<div class="band-profile-content-card">
    ...
    <div id="bp-social-icons-section">
        <h2 style="margin-bottom: 0.5em;"><?php esc_html_e( 'Social Icons', 'extra-chill-community' ); ?></h2>
        <p class="description extrch-sync-info" style="margin-top: -0.5em; margin-bottom: 1em;"><small><?php esc_html_e( 'These icons are also used for your Extrachill.link page and are managed there.', 'extra-chill-community' ); ?></small></p>
        <?php 
        ...
        <button type="button" id="bp-add-social-icon-btn" class="button button-secondary bp-add-social-icon-btn">
            <i class="fas fa-plus"></i> <?php esc_html_e('Add Social Icon', 'extra-chill-community'); ?>
        </button>
    </div>
</div>
*/ ?>
