<?php
/*
 * Template Name: Upvote Feed
 * Description: A page template to show either topics or replies that the user has upvoted.
 */
...
if ($isUserProfile) {
    ...
    echo '<h1>Upvoted Posts</h1>'; // Changed title for clarity
}
...
$posts_query = extrachill_get_upvoted_posts($post_type);
    ...
    <div class="following-feed">
        <form method="get" class="bbp-topics-front-page">
            <select name="filter" id="filter" onchange="this.form.submit()">
                ...
                <option value="extrachill_articles" <?php selected($filter, 'extrachill_articles'); ?>>Show Extra Chill Articles</option>
            </select>
        </form>
...
if ($filter === 'extrachill_articles') {
    // Assuming fetch_extrachill_upvoted_posts() is a function that fetches upvoted articles from Extra Chill
    $extrachill_posts = fetch_extrachill_upvoted_posts(get_current_user_id());
    if (!empty($extrachill_posts)) {
        echo '<div class="extrachill-upvoted-posts">';
        foreach ($extrachill_posts as $post) {
            echo '<div class="post">';
            echo '<h3><a href="' . esc_url('https://extrachill.com/' . $post['slug']) . '">' . esc_html($post['post_title']) . '</a></h3>';
            // Format the date as needed, here it's assumed to be in 'Y-m-d' format
            echo '<p>Date: ' . esc_html(date('F j, Y', strtotime($post['post_date']))) . '</p>';
            echo '<p>By ' . esc_html($post['author_display_name']) . '</p>';
            echo '</div>';
        }
        echo '</div>';
    } else {
        echo '<p>No upvoted Extra Chill articles found.</p>';
    }
}
 else {
                ...
                <ul id="bbp-forum-<?php echo $current_user_id; ?>" class="bbp-topics">
                    <?php while ($posts_query->have_posts()) : $posts_query->the_post();
                            switch ($post_type) {
                                case 'topic':
                                    $topic_id = get_the_ID();
                                    // Fetch dynamic reply count
                                    $reply_count = get_posts([
                                        'post_type' => 'reply',
                                        'post_status' => 'publish',
                                    ...
                                    $unique_voices = count(array_unique(wp_list_pluck($reply_count, 'post_author')));
                                    $reply_count = count($reply_count);

                                    // Fetch last reply details
                                    ...
                                    $last_reply_author = !empty($last_reply) ? get_the_author_meta('display_name', $last_reply[0]->post_author) : 'N/A';
                                    $last_reply_date = !empty($last_reply) ? $last_reply[0]->post_date : 'N/A';
                    ?>
                                <li id="bbp-topic-<?php bbp_topic_id(); ?>" <?php bbp_topic_class(); ?>>                                    <div class="bbp-topic-title">
                                        <?php
                                        ...
                                        <div class="upvote">
                                            <span class="upvote-icon" data-post-id="<?php echo $topic_id; ?>" data-type="topic" data-nonce="<?php echo wp_create_nonce('upvote_nonce'); ?>" role="button" aria-label="Upvote this topic">
                                                <i class="<?php echo $icon_class; ?> fa-circle-up"></i>
                                            </span>
                                            <span class="upvote-count"><?php echo get_upvote_count($topic_id); ?></span> |
                                        </div>
                            ...
                            <div class="bbp-meta">
                                <div class="upvote-date">
                                    ...
                                    <div class="upvote">
                                        ...
                                        <span class="upvote-count"><?php echo get_upvote_count($reply_id); ?></span> |
