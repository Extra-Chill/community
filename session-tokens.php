...
setcookie('ecc_user_session_token', $token, [
    'expires' => time() + (10 * YEAR_IN_SECONDS), // Example: 10 years
    'path' => '/',
...
function store_user_session($token, $user_id) {
    global $wpdb;
    $table_name = $wpdb->prefix . 'user_session_tokens';
$expiration_mysql = date('Y-m-d H:i:s', time() + (10 * YEAR_IN_SECONDS)); // Example: 10 years from now
    ...
    $wpdb->replace(
        ...
        ['%s', '%d', '%s'] // Value formats: token as string, user_id as integer, expiration as string
    );
}
...
function auto_login_via_session_token() {
    ...
    if ($user_id) {
        ...
        if ($user) {
            ...
            // Now passing both required arguments to the wp_login action.
            do_action('wp_login', $user->user_login, $user);
        }
    } else {
        // Token is invalid or expired, consider logging this event or taking additional actions.
    }
}
