<?php
// Professional Profiles Management Page

// Check if the user already has an professional profile
$current_user = wp_get_current_user();
$existing_post_id = wp_surgeon_has_profile_post($current_user->ID, 'professional_profile');

// Fetch the existing profile if it exists
$professional_profile = $existing_post_id ? get_post($existing_post_id) : null;
$title = $professional_profile ? $professional_profile->post_title : '';
$content = $professional_profile ? $professional_profile->post_content : '';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        var apiRoute = '<?php echo esc_url(rest_url('extrachill/v1/manage_professional_profile')); ?>';
...
// Register the REST API route for managing professional profiles
add_action('rest_api_init', function () {
    register_rest_route('extrachill/v1', '/manage_professional_profile', array(
        'methods' => 'POST',
        'callback' => 'handle_manage_professional_profile_request',
        'permission_callback' => function () {
            return is_user_logged_in();
        }
    ));
});
if (!function_exists('handle_manage_professional_profile_request')) {

function handle_manage_professional_profile_request(WP_REST_Request $request) {
...
$success_message = $is_update ? 'Professional profile updated successfully' : 'Professional profile created successfully';
