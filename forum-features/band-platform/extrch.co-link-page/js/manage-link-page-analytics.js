...
(function() {
    ...
    function fetchAnalyticsData() {
        if (!ExtrchLinkPageManager || !ExtrchLinkPageManager.ajaxConfig || !ExtrchLinkPageManager.ajaxConfig.link_page_id) {
            console.error('Link Page ID or AJAX config not available.');
        ...
        const data = {
            ...
            security_nonce: ExtrchLinkPageManager.ajaxConfig.nonce, // Reuse nonce from main form
        ...
        jQuery.post(ExtrchLinkPageManager.ajaxConfig.ajax_url, data, function(response) {
    ...
    // Initial check on DOMContentLoaded in case the analytics tab is the default active one.
    // The main tab script (ui-utils) will also call handleAnalyticsTabBecameVisible if it makes analytics active.
    document.addEventListener('DOMContentLoaded', () => {
        // A brief delay to ensure any initial tab activation by ui-utils.js has occurred
        // and the display style of the analytics tab content is correctly set.
        setTimeout(() => {
            // Check if analytics tab content is visible (e.g., was set as default active)
            const anContent = document.getElementById('manage-link-page-tab-analytics');
            if (anContent && anContent.style.display !== 'none' && !analyticsInitialized) {
                console.log('Analytics tab active on DOMContentLoaded, calling handleVisibility.');
                handleAnalyticsTabBecameVisible();
            }
        }, 300); // Increased delay slightly to ensure other scripts complete initial setup
    });
