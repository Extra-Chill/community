...
(function(manager) {
    ...
    manager.contentPreview.renderSocials = function(socialsArray, previewEl, contentWrapperEl) {
        ...
        socialsArray.forEach(socialData => {
            ...
            let iconClass = socialData.icon; // Use pre-defined icon class if available
            if (!iconClass) {
                // Basic heuristic for Font Awesome class construction
                // This might need to be more robust depending on the variety of 'type' values.
                let faBrandPrefix = 'fab'; // Default for most social icons
                const typeLower = socialData.type.toLowerCase();

                // Specific cases or adjustments can be added here
                if (typeLower === 'email' || typeLower === 'website') {
                    faBrandPrefix = 'fas'; // Font Awesome Solid for generic icons
                }
                if (typeLower === 'spotify') {
                     // fab fa-spotify is correct
                } else if (typeLower === 'applemusic') {
                    iconClass = 'fab fa-apple'; // Or a more specific Apple Music icon if available
                } else if (typeLower === 'email') {
                    iconClass = 'fas fa-envelope';
                } else if (typeLower === 'website') {
                    iconClass = 'fas fa-link';
                } else {
                    // Default construction for brands like instagram, facebook, twitter, etc.
                    iconClass = `${faBrandPrefix} fa-${typeLower.replace(/[^a-z0-9-]/g, '')}`;
                }
            }
    ...
    manager.contentPreview.updatePreviewProfileImage = function(newImageUrl, previewEl) {
        ...
        const imageElement = previewEl.querySelector(PREVIEW_PROFILE_IMAGE_SELECTOR);
        const imageContainer = previewEl.querySelector(PROFILE_IMAGE_CONTAINER_SELECTOR);

        if (imageElement) {
            imageElement.src = newImageUrl;
            imageElement.style.display = newImageUrl ? 'block' : 'none'; // Show/hide based on URL
            if(imageContainer) imageContainer.style.display = newImageUrl ? 'block' : 'none';
        } else {
            console.warn('[ContentRenderer-Info] Profile image element (' + PREVIEW_PROFILE_IMAGE_SELECTOR + ') not found in preview DOM.');
        }
    };
    ...
    manager.contentPreview.removePreviewProfileImage = function(previewEl) {
        ...
        if (imageElement) {
            imageElement.src = ''; // Or set to a placeholder image URL
            imageElement.style.display = 'none';
            if(imageContainer) imageContainer.style.display = 'none';

        } else {
            console.warn('[ContentRenderer-Info] Profile image element (' + PREVIEW_PROFILE_IMAGE_SELECTOR + ') not found in preview DOM for removal.');
        }
    };
