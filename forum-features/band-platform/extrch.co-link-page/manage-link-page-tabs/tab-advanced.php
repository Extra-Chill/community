...
?>
<h2><?php esc_html_e('Advanced Settings', 'generatepress_child'); ?></h2>
...
<div class="bp-link-settings-section">
    ...
    <label style="display:flex;align-items:center;gap:0.5em;font-weight:600;">
        ...
        <?php esc_html_e('Enable Weekly Performance Email', 'generatepress_child'); ?> <span style="font-size:0.95em;color:#aaa;">(coming soon)</span>
    </label>
    ...
    <label style="display:flex;align-items:center;gap:0.5em;font-weight:600;">
        ...
        <?php esc_html_e('Enable Temporary Redirect', 'generatepress_child'); ?> <span style="font-size:0.95em;color:#aaa;">(coming soon)</span>
    </label>
    ...
    <div id="bp-temporary-redirect-target-container" style="margin:0.5em 0 1.5em 1.8em; <?php echo $redirect_enabled ? '' : 'display:none;'; ?>">
        ...
        <select name="link_page_redirect_target_url" id="bp-temporary-redirect-target" style="min-width: 300px;" disabled>
            <option value=""><?php esc_html_e('-- Select a Link --', 'generatepress_child'); ?></option>
            <?php
            // Placeholder: This dropdown should be populated dynamically via JS
            // based on the links configured in the "Links" tab.
            // Example structure:
            // $links_data = get_post_meta($current_link_page_id, '_link_page_links', true);
            // if (is_array($links_data)) {
            //     foreach ($links_data as $section) {
            //         if (isset($section['links']) && is_array($section['links'])) {
            //             foreach ($section['links'] as $link) {
            //                 if (!empty($link['link_url']) && !empty($link['link_text'])) {
            //                     echo '<option value="' . esc_attr($link['link_url']) . '" ' . selected($redirect_target_url, $link['link_url'], false) . '>' . esc_html($link['link_text']) . '</option>';
            //                 }
            //             }
            //         }
            //     }
         ...
         <label style="display:flex;align-items:center;gap:0.5em;font-weight:600;margin-top:1.2em;">
             <input type="checkbox" name="link_page_enable_highlighting" id="bp-enable-link-highlighting" value="1" disabled /> <?php // Placeholder - Fetch real value later ?>
             <?php esc_html_e('Enable Link Highlighting Feature', 'generatepress_child'); ?> <span style="font-size:0.95em;color:#aaa;">(coming soon)</span>
         </label>
         <p class="description" style="margin:0.5em 0 1.5em 1.8em; color:#aaa; font-size:0.97em;"><?php esc_html_e('Allows you to visually emphasize specific links in the "Links" tab.', 'generatepress_child'); ?></p>
    ...
    <h3 style="font-weight:600; font-size: 1.1em; margin-bottom: 0.8em;"><?php esc_html_e('Tracking Pixels', 'generatepress_child'); ?></h3>
